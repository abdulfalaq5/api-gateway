...
echo "🚀 Deploy Kong Routes Script (Instance 2)"
echo "=========================================="

# Kong Instance 2 Configuration
KONG_ADMIN_PORT="9589"
KONG_PROXY_PORT="9588"
KONG_ADMIN_GUI_PORT="9590"
KONG_CONTAINER_NAME="kong-gateway2"
KONG_COMPOSE_FILE="docker-compose.server2.yml"
...
deploy() {
    ...
    # Step 3b: Copy docker-compose.server2.yml to server
    print_status "Step 3b: Copying docker-compose.server2.yml to server..."
    copy_to_server "docker-compose.server2.yml" "$SERVER_DIR/docker-compose.server2.yml"
    print_success "Docker compose file copied to server"
    echo ""
    ...
    print_status "Step 5: Reloading Kong configuration (Instance 2)..."
    
    # Try hot reload first (faster)
    print_status "Attempting hot reload..."
    if execute_remote "curl -s -X POST http://localhost:$KONG_ADMIN_PORT/config -F config=@config/kong.yml 2>&1"; then
        print_success "Hot reload successful! ✓"
        sleep 2
    else
        print_warning "Hot reload failed, restarting container..."
        execute_remote "docker-compose -f $KONG_COMPOSE_FILE restart kong"
        sleep 15
    ...
    print_status "Step 6: Verifying Kong health (Instance 2)..."
    for i in {1..20}; do
        if execute_remote "curl -s http://localhost:$KONG_ADMIN_PORT/status >/dev/null 2>&1"; then
    ...
    print_status "Step 7: Current routes on server (Instance 2):"
    execute_remote "curl -s http://localhost:$KONG_ADMIN_PORT/routes | jq '.data | length' | xargs echo 'Total routes:'"
    echo ""
    
    execute_remote "curl -s http://localhost:$KONG_ADMIN_PORT/routes | jq -r '.data[] | \"  - \" + .name' | head -10"
    echo "  ..."
    echo ""
    ...
    echo "  curl -v http://localhost:$KONG_PROXY_PORT/api/your-endpoint"
}
...
check_status() {
    print_status "📊 Checking Kong status on server (Instance 2)..."
    echo ""
    
    # Kong health
    print_status "Kong Health:"
    execute_remote "curl -s http://localhost:$KONG_ADMIN_PORT/status | jq '.'"
    echo ""
    
    # Routes count
    print_status "Routes:"
    execute_remote "curl -s http://localhost:$KONG_ADMIN_PORT/routes | jq '.data | length' | xargs echo 'Total:'"
    echo ""
    
    # Services count
    print_status "Services:"
    execute_remote "curl -s http://localhost:$KONG_ADMIN_PORT/services | jq '.data | length' | xargs echo 'Total:'"
    echo ""
    
    # Container status
    print_status "Container Status:"
    execute_remote "docker ps --filter name=$KONG_CONTAINER_NAME --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'"
    echo ""
}
...
test_route() {
    ...
    execute_remote "curl -v http://localhost:$KONG_PROXY_PORT$route_path -H 'Content-Type: application/json' -d '{}'"
}
...
show_help() {
    ...
    echo ""
    echo "Commands:"
    echo "  deploy              - Deploy local kong.yml to server (Instance 2)"
    echo "  pull                - Pull kong.yml from server to local"
    echo "  diff                - Show differences between local and remote"
    echo "  status              - Check Kong status on server (Instance 2)"
    echo "  test <path>         - Test a specific route"
    echo "  validate            - Validate local kong.yml"
    echo "  help                - Show this help"
    echo ""
    echo "Instance 2 Configuration:"
    echo "  Container: $KONG_CONTAINER_NAME"
    echo "  Proxy Port: $KONG_PROXY_PORT"
    echo "  Admin Port: $KONG_ADMIN_PORT"
    echo "  Admin GUI Port: $KONG_ADMIN_GUI_PORT"
    echo ""
